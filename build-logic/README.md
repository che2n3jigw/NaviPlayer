...
该模块包含了项目中所有子模块通用的构建逻辑。它利用 Gradle 的约定插件（Convention Plugins）来集中管理和配置通用的构建行为，从而避免在每个子模块的 `build.gradle.kts` 文件中重复编写相同的配置。

## 实现方式

`build-logic` 模块的核心在于 `convention` 子模块，该子模块负责定义和注册一系列自定义的约定插件。这些插件涵盖了 Android 应用程序、Android 库以及 JVM 库的通用构建配置。
...
这些约定插件通过在 `build-logic/convention/src/main/kotlin` 目录下的相应实现类（例如 `AndroidApplicationConventionPlugin.kt`）来定义具体的构建逻辑。然后，在需要应用这些通用配置的子模块中，只需在 `build.gradle.kts` 文件中通过插件 ID 应用相应的约定插件即可。

例如，在 `app` 模块的 `build.gradle.kts` 文件中，可以通过以下方式应用 `androidApplication` 插件：
...
import com.che2n3jigw.naviplayer.buildlogic.configureKotlinJvm
import org.gradle.api.Plugin
import org.gradle.api.Project
import org.gradle.kotlin.dsl.apply
...
*   **集中管理**: 将通用的构建逻辑集中到一个地方，便于维护和修改。
*   **代码复用**: 避免在多个模块中重复编写相同的构建配置。
*   **构建脚本简化**: 使子模块的 `build.gradle.kts` 文件更加简洁，只关注其特定的构建需求。
